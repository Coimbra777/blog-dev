@php
    ...
    $metaDescription = __('blog.blog.tag_description');
@endphp
    ...
    <section class="space-y-10">
        ...
        @if ($posts->isEmpty())
            <div class="rounded-3xl border border-dashed border-white/15 bg-[#252525]/60 px-8 py-16 text-center">
                <h2 class="text-2xl font-semibold text-[#f9f9f9]">{{ __('blog.blog.no_tag_posts') }}</h2>
                <p class="mx-auto mt-3 max-w-xl text-sm leading-7 text-white/55">
                    {!! str_replace(':tag', '<code class="rounded bg-white/5 px-2 py-1 font-mono text-[#f9f9f9]">'.$tag.'</code>', e(__('blog.blog.tag_hint', ['tag' => ':tag']))) !!}
                </p>
            </div>
        @else
            <div class="space-y-2">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-3">
                                <h2 class="text-2xl font-semibold tracking-tight text-[#f9f9f9] sm:text-3xl">
                                    <a href="{{ route(\App\Support\LocalizedRoute::routeName($currentLocale, 'blog.show'), $post->slug) }}" class="transition hover:text-[#fc8e00] focus:outline-none focus:ring-2 focus:ring-[#fc8e00]/60 focus:ring-offset-2 focus:ring-offset-[#151515]">
                                        {{ $post->title }}
                                    </a>
                                </h2>
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach ($post->tags as $postTag)
                                    <a href="{{ route(\App\Support\LocalizedRoute::routeName($currentLocale, 'blog.tag'), $postTag) }}" class="text-xs font-medium uppercase tracking-[0.2em] text-[#fc8e00]/90 transition hover:text-[#fc8e00] focus:outline-none focus:ring-2 focus:ring-[#fc8e00]/60 focus:ring-offset-2 focus:ring-offset-[#151515]">
                                        #{{ $postTag }}
                                    </a>
                                @endforeach
                            </div>
