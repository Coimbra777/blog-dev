@php
    $metaTitle = __('blog.nav.posts').' | '.config('app.name', 'Laravel');
    $metaDescription = __('blog.blog.description');
@endphp
...
@section('content')
    <section class="space-y-12">
        <div class="space-y-5 border-b border-white/10 pb-10">
            <p class="text-xs font-medium uppercase tracking-[0.32em] text-[#fc8e00]">{{ __('blog.blog.eyebrow') }}</p>
            <div class="space-y-4">
        ...
        @if ($posts->isEmpty())
            <div class="rounded-3xl border border-dashed border-white/15 bg-[#252525]/60 px-8 py-16 text-center">
                <h2 class="text-2xl font-semibold text-[#f9f9f9]">{{ __('blog.blog.no_posts') }}</h2>
                <p class="mx-auto mt-3 max-w-xl text-sm leading-7 text-white/55">
                    {!! str_replace(':path', '<code class="rounded bg-white/5 px-2 py-1 font-mono text-[#f9f9f9]">resources/posts</code>', e(__('blog.blog.no_posts_hint'))) !!}
                </p>
            </div>
        @else
            <div class="space-y-2">
